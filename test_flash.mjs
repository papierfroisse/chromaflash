  ...
  const MINI_GAMES = [
    ...
    { id:'mario',      name:'WORLD 1-1',     icon:'🍄',  maxPts: 1000, time: 15, axis: 'rhythm', verb: 'SAUTE !' },
  ...
  function buildSpot(mg, time) {
    ...
    // Generate odd one out: delta is larger so it's humanly visible
    const deltaTarget = settings.difficulty === 'easy' ? 40 : settings.difficulty === 'hard' ? 15 : 25;
  ...
  function buildHex(mg, time) {
    ...
    // Generate 3 wrong hexes
    const wrong1 = rgbToHex(randomColor(settings.difficulty)).toUpperCase();
    const wrong2 = rgbToHex(randomColor(settings.difficulty)).toUpperCase();
    const wrong3 = rgbToHex(randomColor(settings.difficulty)).toUpperCase();
    const options = [hex, wrong1, wrong2, wrong3].sort(() => Math.random() - 0.5);
    ...
    card.innerHTML = `
      ...
      <div id="hex-input-phase" style="display:none">
        <div class="spot-grid" id="hex-choices" style="grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 20px;"></div>
      </div>
    `;
    gameArea.appendChild(card);

    const container = document.getElementById('hex-choices');
    options.forEach(opt => {
      const btn = document.createElement('button');
      btn.className = 'choice-btn';
      btn.textContent = opt;
      ...
      btn.addEventListener('click', () => {
        if (!canAnswer) return;
        canAnswer = false;
        const isCorrect = opt === hex;
        btn.classList.add(isCorrect ? 'correct' : 'wrong');
        if (!isCorrect) {
          container.querySelectorAll('.choice-btn').forEach(b => {
            if (b.textContent === hex) b.classList.add('correct');
          });
        }
        if (isCorrect) { Sound.good(); onMiniGameEnd(mg.maxPts); }
        else { Sound.wrong(); onMiniGameEnd(0); }
    ...
    const showTimer = setInterval(() => {
      ...
      if (countdown <= 0) {
        ...
        document.getElementById('hex-instruction').textContent = 'Quel était le code HEX ?';
        startTimer(time);
      }
    }, 1000);
  }
  ...
  function buildMatch(mg, time) {
    ...
    card.innerHTML = `
      ...
      <p class="text-sm text-secondary mb-md">Mélange les couleurs pour reproduire la cible !</p>
      <div style="display:flex; justify-content:space-around; align-items:center; margin-bottom:16px">
        <div style="text-align:center;">
          <div style="width:100px;height:60px;background:rgb(${target.r},${target.g},${target.b});border-radius:8px; border:2px solid #000;"></div>
          <span class="text-xs font-mono mt-xs block">CIBLE</span>
        </div>
        <div style="text-align:center;">
          <div id="pm-user-color" style="width:100px;height:60px;background:rgb(128,128,128);border-radius:8px; border:2px solid #000;"></div>
          <span class="text-xs font-mono mt-xs block">TOI</span>
        </div>
      </div>
      
      <div class="pm-sliders" style="display:flex; flex-direction:column; gap:8px; margin-bottom:16px;">
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="color:#FF3B30; font-weight:bold; width:15px;">R</span>
          <input type="range" id="pm-r" class="input w-full" min="0" max="255" value="128">
        </div>
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="color:#34C759; font-weight:bold; width:15px;">G</span>
          <input type="range" id="pm-g" class="input w-full" min="0" max="255" value="128">
        </div>
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="color:#007AFF; font-weight:bold; width:15px;">B</span>
          <input type="range" id="pm-b" class="input w-full" min="0" max="255" value="128">
        </div>
      </div>
      
      <button class="btn btn-primary w-full" id="pm-submit">VALIDER</button>
    `;
    gameArea.appendChild(card);

    const box = document.getElementById('pm-user-color');
    const sr = document.getElementById('pm-r');
    const sg = document.getElementById('pm-g');
    const sb = document.getElementById('pm-b');

    function updateColor() {
      current.r = parseInt(sr.value);
      current.g = parseInt(sg.value);
      current.b = parseInt(sb.value);
      box.style.background = `rgb(${current.r},${current.g},${current.b})`;
    }

    sr.addEventListener('input', updateColor);
    sg.addEventListener('input', updateColor);
    sb.addEventListener('input', updateColor);

    document.getElementById('pm-submit').addEventListener('click', () => {
      if (!canAnswer) return;
      canAnswer = false;
      const { score } = computeScore(target, current, mg.maxPts);
      // Tolerance for win: need at least 60% accuracy for a fast-paced game
      if (score >= mg.maxPts * 0.6) {
        Sound.good(); 
        onMiniGameEnd(score);
      } else {
        Sound.wrong(); 
  ...
  // ─── MINI-GAME: SUPER MARIO 3 (1-1) ─────────────────────────
  function buildMario(mg, time) {
    const card = document.createElement('div');
    card.className = 'mini-card w-full';
    card.innerHTML = `
      <div class="mini-game-header" style="background:#5C94FC; color:#FFF; padding:8px 12px; border-radius:4px; margin-bottom:16px;">
        <div style="font-weight:900; font-family:'Press Start 2P', monospace; font-size:1rem;">
          SUPER BROS 3 - WORLD 1-1
        </div>
      </div>
      <p class="text-sm text-secondary mb-md">Esquive les goombas et saute par-dessus le tuyau ! (Espace)</p>
      
      <div style="position:relative; width:100%; height:220px; overflow:hidden; background:#5C94FC; border-radius:8px; border:4px solid #000; margin-bottom:var(--space-md);">
        <!-- Background Decor -->
        <div style="position:absolute; bottom:40px; left:20px; width:60px; height:30px; background:#F8D8A0; border-radius:30px 30px 0 0; opacity:0.8;"></div>
        <div style="position:absolute; bottom:40px; left:200px; width:40px; height:80px; background:#00A800; border:2px solid #000; border-bottom:none;"></div>
        <!-- Floor -->
        <div style="position:absolute; bottom:0; left:0; width:100%; height:40px; background:#F8D8A0; border-top:4px solid #000; display:flex;">
          <!-- Shadow detail -->
          <div style="width:100%; height:10px; background:rgba(0,0,0,0.1); margin-top:4px;"></div>
        </div>
        
        <div id="mario-world" style="position:absolute; top:0; left:0; width:3000px; height:100%;">
          <!-- Pipe -->
          <div class="m-obs" data-x="800" data-w="50" data-h="60" style="position:absolute; bottom:40px; left:800px; width:50px; height:60px; background:#00A800; border:2px solid #000; border-radius:4px 4px 0 0;"></div>
          <!-- Block -->
          <div style="position:absolute; bottom:120px; left:400px; width:30px; height:30px; background:#F87800; border:2px solid #000;"></div>
          <div style="position:absolute; bottom:120px; left:430px; width:30px; height:30px; background:#E8C000; border:2px solid #000; text-align:center; font-weight:bold; line-height:26px;">?</div>
          
          <!-- Flag -->
          <div style="position:absolute; bottom:40px; left:2000px; width:4px; height:130px; background:#000;">
            <div style="position:absolute; top:10px; right:0; width:40px; height:30px; background:#FFF; border:2px solid #000;"></div>
          </div>
        </div>
        
        <!-- Player -->
        <div id="mario-player" style="position:absolute; bottom:40px; left:60px; width:30px; height:30px; background:#F83800; border:2px solid #000; border-radius:8px 8px 0 0; z-index:10; transition: transform 0.1s;">
          <div style="position:absolute; top:6px; right:4px; width:10px; height:10px; background:#FFA000;"></div>
        </div>
        
        <!-- Enemies -->
        <div id="mario-enemies" style="position:absolute; top:0; left:0; width:100%; height:100%;"></div>
      </div>
      
      <button class="btn btn-primary w-full mt-sm" id="mario-btn">SAUTER (Espace)</button>
    `;
    gameArea.appendChild(card);

    const world = document.getElementById('mario-world');
    const player = document.getElementById('mario-player');
    const enemiesLayer = document.getElementById('mario-enemies');
    ...
    // Spawn 2 goombas
    let goombas = [
      { x: 500, y: 40, w: 30, h: 30, el: null, active: true },
      { x: 1200, y: 40, w: 30, h: 30, el: null, active: true }
    ];
    
    goombas.forEach(g => {
      g.el = document.createElement('div');
      g.el.style.position = 'absolute';
      g.el.style.bottom = g.y + 'px';
      g.el.style.width = g.w + 'px';
      g.el.style.height = g.h + 'px';
      g.el.style.background = '#800000'; // dark brown
      g.el.style.borderRadius = '50% 50% 0 0';
      g.el.style.border = '2px solid #000';
      enemiesLayer.appendChild(g.el);
    });
    
    const obstacles = [
      { x: 800, w: 50, h: 60 } // The pipe
    ];
    ...
    function jump() {
      if (!isJumping && gameState === 'playing' && canAnswer) {
        isJumping = true;
        playerVy = jumpStrength;
        Sound.swoosh(); // jump sound
      }
    }

    document.getElementById('mario-btn').addEventListener('click', jump);
    const keyHandler = (e) => {
      if (!canAnswer) return;
      if (e.key === ' ' || e.key === 'ArrowUp') {
        e.preventDefault();
    ...
    function loop() {
      ...
      if (gameState === 'playing') {
        ...
        const pStart = 60; // Player stays at x=60 visually on screen
        const pEnd = 90;
        const pWorldStart = pStart + scrollX;
        const pWorldEnd = pEnd + scrollX;
        ...
        if (isJumping) {
          ...
          let onPipe = false;
          let hitPipeSide = false;
          
          for (let obs of obstacles) {
            // Check intersection
            if (pWorldEnd > obs.x && pWorldStart < obs.x + obs.w) {
              if (playerY >= 40 + obs.h && playerVy < 0) {
                // Landing on pipe
                playerY = 40 + obs.h;
                playerVy = 0;
                isJumping = false;
                onPipe = true;
              } else if (playerY < 40 + obs.h) {
                // Hit the side of the pipe
                hitPipeSide = true;
              }
            }
          }
          ...
          for (let obs of obstacles) {
            if (pWorldEnd > obs.x && pWorldStart < obs.x + obs.w) {
              if (playerY >= 40 + obs.h) onPipe = true;
              else hitPipeSide = true;
            }
          }
          if (hitPipeSide) {
             gameState = 'lose';
        ...
        goombas.forEach(g => {
          if (!g.active) return;
          // Goomba moves left relative to world
          g.x -= 2; 
          // Position visually (g.x is world coordinate, minus scrollX is screen coordinate)
          const screenX = g.x - scrollX;
          g.el.style.left = screenX + 'px';
          
          // Collision Check
          if (screenX < pEnd && screenX + g.w > pStart) {
            // Horizontal overlap
            if (playerY < g.y + g.h && playerY + 30 > g.y) {
              if (playerVy < -2 && playerY > g.y + 10) {
                // Stomp!
                g.active = false;
                g.el.style.transform = 'scaleY(0.2)';
                playerVy = 10; // bounce
                Sound.click();
              } else {
                // Hit!
        ...
        // Check Win Condition (reach the flag at x=2000)
        if (pWorldEnd >= 2000) {
          gameState = 'win';
        }
