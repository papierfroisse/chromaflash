  ...
  // ─── MINI-GAME: SUPER MARIO 3 (1-1) CANVAS EDITION ──────────────
  function buildMario(mg, time) {
    ...
    card.innerHTML = `
      ...
      <div style="width:100%; height:220px; border-radius:8px; border:4px solid #000; margin-bottom:var(--space-md); overflow:hidden;">
        <canvas id="mario-canvas" width="600" height="220" style="width:100%; height:100%; background:#5C94FC; display:block;"></canvas>
      </div>
    ...
    let scrollX = 0;
    const speed = 4.5; // Fixed pixels per frame scrolling
    
    // Physics tuned for Canvas
    const gravity = 0.6;
    const jumpForce = -10;
    const floorY = 180;
    
    const player = {
      x: 100, // Fixed screen position X
      y: floorY - 30, // Start on floor
      w: 30,
      h: 30,
      vy: 0,
      isJumping: false,
      color: '#F83800', // Mario red
      hat: '#FFA000',
      deadRotation: 0
    };
    
    const goombas = [
      { x: 500, y: floorY - 30, w: 30, h: 30, active: true, vy: 0 },
      { x: 1200, y: floorY - 30, w: 30, h: 30, active: true, vy: 0 }
    ];
    ...
    function jump() {
      if (!player.isJumping && gameState === 'playing' && canAnswer) {
    ...
    function draw() {
      ...
      ctx.fillStyle = '#000';
      ctx.fillRect(0, floorY, canvas.width, 4); // top border
      ctx.fillStyle = 'rgba(0,0,0,0.1)';
      ctx.fillRect(0, floorY + 8, canvas.width, 10); // shadow detail
      
      // Scenery: clouds/bushes
      ctx.fillStyle = 'rgba(255,255,255,0.8)';
      ctx.fillRect(200 - (scrollX * 0.2) % 600, 50, 60, 30);
      ctx.fillRect(500 - (scrollX * 0.2) % 600, 80, 40, 20);
      
      // Obstacles
      for (let obs of obstacles) {
        const screenX = obs.x - scrollX;
        if (screenX + obs.w > -100 && screenX < canvas.width + 100) {
          ctx.fillStyle = '#00A800';
          ctx.fillRect(screenX, floorY - obs.h, obs.w, obs.h);
          ctx.strokeStyle = '#000';
          ctx.lineWidth = 4;
          ctx.strokeRect(screenX, floorY - obs.h, obs.w, obs.h);
          // Pipe lip
          ctx.fillRect(screenX - 4, floorY - obs.h, obs.w + 8, 16);
          ctx.strokeRect(screenX - 4, floorY - obs.h, obs.w + 8, 16);
          // Pipe highlight
          ctx.fillStyle = 'rgba(255,255,255,0.4)';
          ctx.fillRect(screenX + 6, floorY - obs.h + 18, 8, obs.h - 18);
        }
      }
      
      // Flag
      const flagX = 2000 - scrollX;
      if (flagX > -100 && flagX < canvas.width + 100) {
        ctx.fillStyle = '#000';
        ctx.fillRect(flagX, floorY - 130, 6, 130); // pole
        ctx.fillStyle = '#FFF';
        ctx.fillRect(flagX + 6, floorY - 130 + 10, 40, 30); // flag
        ctx.strokeStyle = '#000';
        ctx.lineWidth = 2;
        ctx.strokeRect(flagX + 6, floorY - 130 + 10, 40, 30);
      }
      ...
      for (let g of goombas) {
        ...
        if (screenX + g.w > -50 && screenX < canvas.width + 50) {
          ctx.fillStyle = '#800000';
          ctx.beginPath();
          ctx.arc(screenX + g.w/2, g.y + g.h, g.w/2, Math.PI, 0);
          ctx.lineTo(screenX + g.w, g.y + g.h);
          ctx.lineTo(screenX, g.y + g.h);
          ctx.fill();
          ctx.strokeStyle = '#000';
          ctx.lineWidth = 2;
          ctx.stroke();
          // Eyes
          ctx.fillStyle = '#FFF';
          ctx.fillRect(screenX + 6, g.y + 10, 6, 8);
          ctx.fillRect(screenX + 18, g.y + 10, 6, 8);
        }
      }
      ...
      if (gameState === 'lose') {
        player.deadRotation += 0.2;
        ctx.rotate(player.deadRotation);
      }
      ...
      // Hat/Details
      ctx.fillStyle = player.hat;
      ctx.fillRect(-player.w/2 + 10, -player.h/2 + 4, 12, 8);
      // Eye
      ctx.fillStyle = '#FFF';
      ctx.fillRect(-player.w/2 + 20, -player.h/2 + 10, 4, 6);
      ctx.restore();
    }
    ...
    function update() {
      if (gameState === 'playing') {
        scrollX += speed;
        const pWorldX = player.x + scrollX;
        ...
        // Obstacles collision (AABB)
        for (let obs of obstacles) {
          // Horizontal overlap
          if (pWorldX + player.w > obs.x && pWorldX < obs.x + obs.w) {
            // Check vertical
            if (player.y + player.h <= floorY - obs.h + 15 && player.vy >= 0) {
              // Landing on top
              player.y = floorY - obs.h - player.h;
              player.vy = 0;
              onGround = true;
            } else if (player.y + player.h > floorY - obs.h) {
              // Hit the side
              hitSide = true;
            }
          }
        ...
        if (hitSide) {
          gameState = 'lose';
          player.vy = -8; // death bounce
        }
        
        // Floor collision
        if (!onGround && player.y + player.h >= floorY) {
        ...
        for (let g of goombas) {
          ...
          if (pWorldX + player.w > g.x && pWorldX < g.x + g.w) {
            if (player.y + player.h < g.y + 20 && player.vy > 0) {
              // Stomp
              g.active = false;
              player.vy = -9; // bounce
              Sound.click();
            } else if (player.y + player.h > g.y + 10) {
              // Hit
              gameState = 'lose';
              player.vy = -8;
            }
